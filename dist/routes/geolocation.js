...
geolocationRouter.get('/search', async (req, res) => {
    ...
    try {
        const userRegion = req.headers['x-user-region'] || 'eu';
        const cacheResult = await enhancedCacheService.smartGet(cacheKey, ENDPOINT_GEO_SEARCH, userRegion);
        ...
        if (results && results.length > 0) {
            ...
            await enhancedCacheService.smartSet(cacheKey, result, ENDPOINT_GEO_SEARCH, userRegion);
            await supabase.rpc('log_performance', {
                endpoint_name: ENDPOINT_GEO_SEARCH,
...
geolocationRouter.get('/reverse-geocode', async (req, res) => {
    ...
    try {
        const userRegion = req.headers['x-user-region'] || 'eu';
        const cacheResult = await enhancedCacheService.smartGet(cacheKey, ENDPOINT_GEO_REVERSE, userRegion);
        if (cacheResult.status === 'hit' && cacheResult.data) {
            await supabase.rpc('log_performance', {
        ...
        await enhancedCacheService.smartSet(cacheKey, result, ENDPOINT_GEO_REVERSE, userRegion);
        await supabase.rpc('log_performance', {
            endpoint_name: ENDPOINT_GEO_REVERSE,
            response_time: responseTime,
            cache_status: 'miss'
